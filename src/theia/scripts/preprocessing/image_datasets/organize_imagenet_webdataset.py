...
def create_shard(
    ...
    with wds.TarWriter(shard_path) as tar_writer:
        for i, (image, basename, text) in enumerate(frames):
            image_out = BytesIO()
            np.save(image_out, image)
            sample = {"__key__": basename, "image": image_out.getvalue(), "text": text}
...
def main() -> None:
    ...
    image_paths, texts = [], []
    with open('/home/xshadow/Datasets/remote_clip/csv_file/train/Seg-4.csv', newline='') as csvfile:
        spamreader = csv.reader(csvfile, delimiter='\t', quotechar='|')
        for row in spamreader:
            if row[0] == "title":
                continue
            texts.append(row[0])
            image_paths.append(os.path.join(IMAGE_DATASET_RAW_DIR,row[1]))
    ...
    for image_path, text in zip(image_paths,texts):
        ...
        shard_buffer.append((image, basename, text))
